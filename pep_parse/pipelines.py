...
class PepParsePipeline:
    ...
    # Логика:
    # в методе __init__ - задем адрес расположения будущего файла со сводкой по статусам

    # в методе open_spider - подготовимся к записи спарсенных данных, создаем defaultdict
    # в котором ключами будут статусы, по умолчанию для каждого ключа соответствует 0
    # но по мере прохождения по всем документам кол-во документов
    # по каждому статусу будет увеличиваться

    # в методе process_item - происходит обработка данных по одному элементу за раз
    # берем каждый документ и для него в словаре status_sum по ключу item[status] добавляем 1
    # и так для каждого документа (каждый объект Item)
    # в итоге получаем словарь status_sum c полным перечнем всех статусов
    # и кол-ву документов с этим статусом

    # в методе close_spider - завершаем экспорт данных в csv-файл
    # создаем файл и с помощью контекстного менеджера и библиотеки csv
    # запишем необходимые данные в файл в нужном виде:
    # сначала "шапка" таблицы
    # потом построчно статусы и кол-во док-ов
    # потом финальная строка с общим кол-вом док-ов
